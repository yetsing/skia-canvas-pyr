...
use crate::filter::{Filter, FilterQuality, ImageFilter};
...
use crate::typography::{Baseline, DecorationStyle, FontSpec, Spacing, Typesetter};
use crate::utils::*;
...
impl Context2D {
  ...
  pub fn paint_for_drawing(&mut self, style: PaintStyle) -> Paint {
    ...
    self
      .state
      .dye(style)
      .mix_into(&mut paint, self.state.global_alpha, self.state.image_filter);
...
#[derive(FromPyObject)]
pub enum DyeValue<'a> {
  Gradient(PyRef<'a, CanvasGradient>),
  Pattern(PyRef<'a, CanvasPattern>),
  Texture(PyRef<'a, CanvasTexture>),
  Color(String),
}

impl Dye {
  pub fn new<'a>(value: DyeValue<'a>) -> Option<Self> {
    match value {
      DyeValue::Gradient(gradient) => Some(Dye::Gradient(gradient.clone())),
      DyeValue::Pattern(pattern) => Some(Dye::Pattern(pattern.clone())),
      DyeValue::Texture(texture) => Some(Dye::Texture(texture.clone())),
      DyeValue::Color(color) => css_to_color(&color).map(Dye::Color),
    }
  }

  pub fn value(&self) -> Option<String> {
    match self {
      Dye::Color(color) => Some(color_to_css(color)),
      _ => None,
    }
  }
