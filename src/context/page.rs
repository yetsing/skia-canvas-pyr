...
use crate::utils::{css_to_color, to_color_type};
      ...
      _ => {
        ...
        if self.depth() > cache_depth {
          if rayon::current_thread_index().is_some() {
            ...
            image
              .make_non_texture_image(&mut surface.direct_context())
  ...
  pub fn materialize(id: usize, engine: &RenderingEngine, options: &ExportOptions) {
    Self::shared().get_mut(&id).map(|mut cache| {
      ...
      if let Some(ref img) = cache.image {
        if !cache.is_valid(options) || !img.is_texture_backed() {
          return;
        }
      }
...
pub fn pages_arg(
  ...
  opts: &ExportOptions,
  canvas: &mut Canvas,
) -> PyResult<PageSequence> {
  let engine = canvas.engine();
  let pages = pages
    .iter()
    .map(|ctx| ctx.get_page_for_export(opts, &engine))
    .collect();
...
impl FromPyObject<'_, '_> for ExportOptions {
  type Error = PyErr;

  fn extract(obj: Borrowed<'_, '_, PyAny>) -> Result<Self, Self::Error> {
    let dict = obj.cast::<PyMapping>()?;

    let format: String = dict.get_item("format")?.extract()?;
    let quality: f32 = dict.get_item("quality")?.extract()?;
    let density: f32 = dict.get_item("density")?.extract()?;
    let jpeg_downsample: bool = dict.get_item("downsample")?.extract()?;
    let matte = dict
      .get_item("matte")
      .ok()
      .and_then(|v| v.extract::<String>().ok())
      .and_then(|v| css_to_color(&v));
    let msaa = dict
      .get_item("msaa")
      .ok()
    ...
    let color_type = dict
      ...
      .ok()
      .and_then(|v| v.extract::<String>().ok())
      .map(|v| to_color_type(&v))
      .unwrap_or(ColorType::RGBA8888);
    let text_contrast: f32 = dict.get_item("text_contrast")?.extract()?;
    let text_gamma: f32 = dict.get_item("text_gamma")?.extract()?;
    let outline: bool = dict.get_item("outline")?.extract()?;
