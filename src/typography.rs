...
impl Typesetter {
  ...
  pub fn metrics(&self) -> Value {
    ...
    let lines = (0..paragraph.line_number())
      .filter_map(|ln| {
        ...
        let half_leading =
          self.graf_style.strut_style().leading().max(0.0) * self.char_style.font_size() / 2.0;
        ...
        let font_runs = text_runs
          .iter()
          .filter(|r| r.line == ln)
          .collect::<Vec<&TextRun>>();
        let text_bounds = font_runs
          .iter()
          .map(|run| run.bounds)
          .reduce(Rect::join2)
          .unwrap_or(Rect::new_empty());
...
pub fn typeface_details(filename: &str, font: &Typeface, alias: Option<String>) -> TypefaceDetails {
  ...
  TypefaceDetails {
    family,
    weight: *style.weight(),
    slant: from_slant(style.slant()),
    width: from_width(style.width()),
