...
use crate::typography::{
  FontSpec, TypefaceDetails, from_slant, from_width, typeface_details, typeface_wght_range,
};
...
#[pyfunction]
pub fn get_families() -> Vec<String> {
  FontLibrary::with_shared(|lib| lib.families())
}
...
#[pyfunction]
pub fn has(family_name: String) -> bool {
  FontLibrary::with_shared(|lib| lib.families().contains(&family_name))
}

#[pyfunction]
pub fn family(family_name: String) -> Option<FamilyDetails> {
  let (weights, widths, styles) = FontLibrary::with_shared(|lib| lib.family_details(&family_name));
  ...
  for filename in filenames.iter() {
    ...
    match typeface {
      Some(font) => {
        ...
        let details = typeface_details(filename, &font, alias.clone());
